...
Relying on the ODU's 3-minute delay as the anti-short-cycle mechanism **for the automatic control loop** is **not acceptable** — repeated trips latch faults, and the split of protection responsibility between ODU, interface board, and thermostat in communicating mode is an **open question** (verify during Phase 2; see docs/05). Our software min-OFF therefore stays **primary for every AUTOMATIC start** (CompressorGuard, plus the StagedCoolShaper's own demand-level rest on the cool channel).

**The one sanctioned exception — a deliberate human request.** A user-initiated setpoint/mode/preset change bypasses the software min-OFF (both CompressorGuard and the cool shaper's demand-level rest) so the demand issues immediately, leaning on the ODU's ~3-min restart delay purely as the *physical* backstop. This does **not** reintroduce the fault-latching risk above, because it is **not** an automatic mechanism that can oscillate: it fires only on an explicit one-shot armed by the user action (expiring in ~2 min), and — critically — the **max-starts/hour cap is NOT bypassed** (3/h at the guard, 2/h at the cool shaper, both still enforced on manual starts). The starts cap is what bounds any restart pattern away from the repeated-trip regime that latches an ODU fault; boot hold-off and the reset-loop lockout are likewise never bypassed. min-ON, mutual exclusion, and all failsafe stops remain unconditional. Rationale: the min-OFF exists to stop the *automatic* loop from chasing the deadband, not to make a person wait out a timer the equipment already enforces.
...
- **CompressorGuard (new, primary compressor protection).** Enforces min-OFF (**180 s**, set to match the ODU's own ~3-min internal restart delay rather than over-waiting it — §1a; retune if a precise ODU spec is supplied), min-ON (300 s), max 3 starts/hour, and post-boot hold-off. **Timer state persisted in NVS/RTC**; if persisted state is missing/invalid at boot, assume the worst and enforce the full hold-off. No **automatic** code path — including failsafe recovery and mode changeover — may bypass it; the sole exception is a deliberate user-initiated change, which relaxes **min-OFF only** (§1a, starts-cap preserved). On the cool channel a second, longer demand-level rest lives in the StagedCoolShaper (docs/05 staged-cooling row) for long-low modulation / dehumidification and follows the same manual-bypass rule.
...
- **Reduced safe-UI on the reset-loop latch (issue #80):** a crash in a *non-critical* UI/rendering feature must degrade, not boot-loop. When the reset-loop latch trips (§2 row: ≥3 watchdog/brownout resets in 30 min), the wall unit persists an NVS "reduced UI" flag and, on the next boot, builds a **minimal known-good screen** (plain temp + mode + alarms — no chart, mode-tint gradient, or RS-485 LISTEN capture) instead of re-running the code that crashed. The flag **persists until manually cleared** by the on-screen "Restore full screen" button, which clears both the reduced-UI flag and the reset-loop history and reboots. Control/safety are unaffected — they already fail to no-demand on the same latch. Config hardening for the known-bad path is in place: software gradients are drawn as solid fills (the `lv_draw_sw_gradient` cache hang that started this) and the gradient cache is sized in `platformio.ini`.
...
- **Per-sensor fault policy (each remote sensor, all mandatory):** staleness timeout (default 300 s; requires the 60 s HA heartbeat republish — see docs/06; sensor topics **non-retained** so a dead broker can't replay stale temps); range gate (5–40 °C); stuck-value detection (#153 peer-disagreement policy — flat ≠ stuck: 0.1 °C-quantized sensors legitimately repeat for hours in a regulated space; stuck = flat while a majority of peers moved, or flat past an absolute ceiling; docs/05 defaults table); **divergence check** — a sensor > 4 °C from the participant median is excluded + alarmed. Occupancy-driven re-weighting is rate-limited (slew ~0.1 °C/min on the effective input) so a sensor joining/leaving can't step the control input.
