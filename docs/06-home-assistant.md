...
| HA → ESP32 | `slytherm/cmd/smart_recovery` | `ON` / `OFF` — act on the smart-recovery advice (#50). NVS-backed and mirrored **retained** on `slytherm/state/smart_recovery`; exposed as `switch.slytherm_smart_recovery`. The estimator learns either way — this gates only whether the learned lead is used |
...
| ESP32 → HA | `slytherm/state/recovery` | **retained** #183 recovery/coast diagnostics JSON: `{"enabled":bool,"heat_hp_cph":1.00,"heat_gas_cph":1.00,"cool_cph":0.80,"heat_hp_n":0,"heat_gas_n":0,"cool_n":0,"lead_s":0,"applied":"none"/"start"/"coast","coast":bool}` — learned °C/h + accepted-segment count per channel, last advised early-start lead, which arm last early-applied a scheduled setpoint, and the live coast verdict |
...
- **Off by default, switchable at runtime.** `kRecoveryEnabledDefault = false` is only the first-boot value; the live setting is `switch.slytherm_smart_recovery` (`slytherm/cmd/smart_recovery`, NVS-backed, retained state). Leave it off until a few weeks of learned ramp rates look sane for the installed equipment, then turn it on — and turn it back off from HA, without a reflash, if a recovery misbehaves. Learning runs even while disabled, so enabling starts from real data.
- **Ready-made HA side:** [`ha/packages/slytherm_smart_recovery.yaml`](../ha/packages/slytherm_smart_recovery.yaml) publishes `next_target` every minute from the 4-preset daily schedule, reading the setpoints from the thermostat's own retained `config/presets` roster (HA never hard-codes them). It stands down during vacation/away. **It replaces any fixed pre-cool automation** — a hard-coded "pre-cool at 06:30" both fights this one and is wrong whenever the weather is not what it was tuned for.
- **Coast — the mirror arm (#183, optimal stop):** when `next_target` *relaxes* the active side (evening cool setpoint rising, night heat setpoint falling), the firmware early-applies the relaxed setpoint once the live drift slope shows the room cannot overshoot it before the boundary — instead of defending the outgoing band to the last minute (field data: ~an hour of pointless compressor runtime on half of weekday evenings). Window-bounded at **1 h** (`kCoastMaxLeadS`) so a tight house never trades hours of the outgoing comfort band for the release; an untrusted or implausible drift slope *disables* the release, never enables it. Diagnostics on retained `slytherm/state/recovery` (topic map above).
- **One feature, one switch — "Smart setpoints".** Early-start (both directions) and coast are a single user-facing feature (ecobee's Smart Recovery, competitive P-row): *arrive at the future setpoint on time by cooling, heating, or doing nothing*. Governed solely by the #50 runtime switch (`slytherm/cmd/smart_recovery`, HA entity display name **"SlyTherm Smart Setpoints"**, `unique_id` unchanged), and visible/toggleable on every panel: the **System Mode sheet** (Settings → System Mode) carries a "Smart setpoints" toggle — Controller applies locally, Remotes forward over the cmd topic and re-seed from the retained state (the #128 fan-sheet contract). The Settings "System Mode" card summary shows the live state. `kCoastEnabledDefault` is `true`; it remains only as an emergency compile-out.
...
- **Scope — the screen lock is PER-DEVICE.** Each screen (the wall Controller and every Remote) has its own independent PIN and lock, persisted in that device's own NVS; a lock is **not** replicated over the `#102` link (which syncs setpoints/mode/hold/preset/temp only) and does **not** gate HA/MQTT control at all. So PIN-locking a Remote locks only that Remote's touchscreen.
- **Forgotten-PIN recovery:** publish exactly `clear_user_pin` to `slytherm/cmd/lock_clear`. The user PIN is cleared and a user lock released — **no PIN is required** — on the **Controller and every Remote at once** (all subscribe to the topic, so one command recovers any device's lock; issue #45 extended to remotes). Rationale: anyone who can publish to the broker already has full climate control (every setpoint, mode, and EM HEAT) — **HA/broker access = admin by definition**; a PIN gate here would add no security, only a second thing to forget. HA stays a comfort/visibility layer either way — the lock is tamper resistance, not a safety mechanism.
