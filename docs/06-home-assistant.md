...
- **Coast — the mirror arm (#183, optimal stop):** when `next_target` *relaxes* the active side (evening cool setpoint rising, night heat setpoint falling), the firmware early-applies the relaxed setpoint once the live drift slope shows the room cannot overshoot it before the boundary — instead of defending the outgoing band to the last minute (field data: ~an hour of pointless compressor runtime on half of weekday evenings). Window-bounded at **1 h** (`kCoastMaxLeadS`) so a tight house never trades hours of the outgoing comfort band for the release; an untrusted or implausible drift slope *disables* the release, never enables it. Rides the same `smart_recovery` switch; additionally compile-gated `kCoastEnabledDefault = false` until field-tuned. Diagnostics on retained `slytherm/state/recovery` (topic map above).
