...
| Fused-temp coast grace (#153) | **120 s** (`kFusionCoastMaxS`, range 0–600; 0 disables) | fusion JUST lapsed (all participants stale/stuck — field case: the stuck window expiring on flat overnight temps) + last-good younger than the grace + inside the 5–40 °C plausibility band → keep reporting last-good, flagged `coasting` (tier/degraded carried, so a local-degraded coast keeps its cooling lockout), instead of forcing a safety stop; hard-invalid beyond the grace or at boot (no last-good) — a real prolonged failure still fails to no-demand |
