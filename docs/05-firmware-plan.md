...
*Instrumentation (`-DSLYTHERM_TXTURN_PROBE`, thermostat_s3; passive/ship-safe — no TX, no behavior change, rides shadow windows automatically):* a probe in `ct485Task` times, on every coordinator grant addressed to the slot SlyTherm occupies (OEM thermostat = **node 1** = `kAddrThermostat`; R2R `0x00` w/ dataflow bit, or Token Offer `0x77`), the firmware's **compute** turnaround `t0 → t1`: `t0` = the grant just decoded in the RX loop; `t1` = the would-be node-1 reply **built + encoded**, ready for DE to assert. The reply is `Ct485Thermostat::dryRunGrantResponse()` — a pure/const dry-run of `handleGrant()`'s branch order that touches no state and no bus, so the shadow-mode stack (which never resumes) still times the *real* TX-path work. Samples feed a fixed-bucket histogram (`lib/LatencyStats`, host-unit-tested); a 30 s telnet `[txturn]` line reports `n / min / p50 / p95 / p99 / p999 / max` (µs) plus `w2wmax` = compute max + the two components software can't see (task-tick dwell ≤ `kCt485TickMs`; DE pre-drive `kDePrePostUs`). Compare `w2wmax` — **not** the bare compute number — against the ~123 ms OEM node-1 floor / 3000 ms coordinator timeout.

*Run procedure (execute AFTER a shadow-validation window closes — the probe rides the window passively, but read the verdict when the controller is free):*
1. **Passive baseline** (default): probe is on from boot. Leave the controller running its normal shadow load ≥ **1 h** (≈ 1100+ grants at 3.2 s) so p999 populates; `telnet <ip> 23`, watch the `[txturn]` line. Record the last (highest-`n`) line's `p99`, `max`, `w2wmax`.
2. **Worst-case stress**: `mosquitto_pub -t slytherm/cmd/txturn_stress -m on` — this hammers the heaviest LVGL path (continuous full-screen 800×480 redraws on core 0, ~5 ms cadence) to maximize PSRAM/RGB contention against the core-1 probe. **Flipping the toggle auto-clears the histogram** (edge-triggered `gTxTurn.reset()` in `ct485Task`), so the stress-run `[txturn]` line is *pure stress* — no reboot needed and passive samples never dilute the stress p99. Run ≥ **1 h**, record the same fields. `-m off` restores normal rendering (and clears again for a clean passive reading); the toggle is never persisted, so any reboot also returns to a clean passive baseline.
3. **Verdict:** pass if stress-run `w2wmax` stays well under 123 ms (expected: dwell-dominated ≈ `kCt485TickMs` + sub-ms compute ≈ 20–25 ms, ~5–6× margin). Exceed → split architecture (already de-risked; the P4 Remote runs display-over-MQTT in production). Record pass/fail on issue #28.
...
|---|---|---|
| Compressor min OFF time | **180 s** | 180–900 s; set to match the ODU's own ~3-min internal restart delay (retune if a precise spec is supplied) rather than over-waiting it — ours is still primary for **automatic** starts. A deliberate **user-initiated** setpoint/mode/preset change bypasses this min-OFF (one-shot, ~2 min window; [`04-safety.md`](04-safety.md) §1a/§3), leaning on the ODU delay as the physical backstop; **max-starts/hour, boot hold-off, min-ON, and the reset-loop lockout are NOT bypassed** |
...
| Staged cooling demand shaping (#140) | stage **30%** (`kCoolStagePct`), min run **780 s** / min rest **480 s** (`kCoolMinOnS`/`kCoolMinOffS`), starts cap **2/h** (`kCoolMaxStartsPerH`), base duty period **4500 s** (`kCoolCyclePeriodS`), proportional band top **0.45 °C** (`kCoolFullDutyErrC`), manual-arm window **120 s** (`kCoolManualArmWindowS`) | field-confirmed single stage engaged at CT-485 demand 30% — request is a RUNTIME fraction (StagedCoolShaper): error→duty→slow cycling; min-run gates **comfort** stops only (invalid input drops immediately, gate still consulted); CompressorGuard timers stay downstream; defaults derived from the 2026-07-09/10 shadow-vs-OEM data (test/test_cool_replay). **The 480 s min rest is an AUTOMATIC-loop timer, not a compressor-safety timer** (the ODU ~3-min delay + CompressorGuard are that) — it exists for long-low modulation + wet-coil dehumidification (short cycles push effective SHR→1.0, [`13-dual-fuel-control-research.md`](13-dual-fuel-control-research.md) §4/§8). A **manual** setpoint-down (#151) bypasses this min rest via `StagedCoolShaper::armManual` (one-shot, arm-window above) so a deliberate human cool request isn't held up to 8 min; **min-ON, the 2/h starts cap, and the gate are all preserved** on manual starts (no coil-freeze/condensate hazard — those are handled by min-ON, the 18 °C indoor lockout, blower-proven, and the hardwired float switch, none of which is a restart-timing gate) |
...
| TX-turnaround probe report cadence (#28) | **30 s** (`kTxTurnReportMs`) | passive/ship-safe jitter probe (`-DSLYTHERM_TXTURN_PROBE`, thermostat_s3): telnet `[txturn]` line cadence. Probe samples the firmware's grant→DE-ready **compute** turnaround (dry-run reply build+encode, no bus drive) on every R2R/Token grant to node 1 (~3.2 s); reference budgets `kTxTurnOemFloorUs` = 123 000 µs (OEM node-1 grant→reply floor), `kTxTurnTimeoutUs` = 3 000 000 µs (coordinator give-up). Wire-to-wire = compute + task-tick dwell (≤ `kCt485TickMs` FIFO wait) + `kDePrePostUs` |
...
| Smart recovery (pre-heat/pre-cool) | **disabled at first boot** (`kRecoveryEnabledDefault`), runtime-switchable (`slytherm/cmd/smart_recovery`, NVS `smrec`) | ADVISORY only — RecoveryEstimator recommends an early start for the next scheduled target (`slytherm/cmd/next_target`); ModeStateMachine/glue decides, CompressorGuard + DualFuelArbiter still gate every demand. The switch gates only whether the advice is ACTED on; learning runs regardless, so enabling starts from real data ([`06-home-assistant.md`](06-home-assistant.md) "Smart recovery") |
...
| Coast / optimal stop (#183) | **enabled** (`kCoastEnabledDefault`; owner decision 2026-08-08 — coast is part of the ONE "smart setpoints" feature, governed solely by the #50 `smart_recovery` runtime switch; the constant remains an emergency compile-out), window **3600 s** (`kCoastMaxLeadS`), margin **0.2 °C** (`kCoastMarginC`), drift clamp **2.0 °C/h** (`kCoastDriftMaxCPerH`) | the mirror of smart recovery (ASHRAE optimal STOP): when `next_target` RELAXES the active side, the relaxed setpoint is early-applied once the live TrendEstimator slope projects the room cannot overshoot it (by ≥ margin) before the boundary — instead of defending the outgoing band to the last minute (field data 2026-08: ~1 h of pointless compressor runtime before the 20:00 relax on ~half of weekday evenings). Window-bounded so a tight house (drift ~0.1–0.25 °C/h measured) never trades hours of the outgoing band; an untrusted/implausible slope DISABLES the release; rides the #50 `smart_recovery` runtime switch; ADVISORY — the applied setpoint goes through the normal pipeline, CompressorGuard min-run governs the call exit; diagnostics on retained `slytherm/state/recovery`; **heat side (evening→night drop) is a winter validation task** |
