...
| `0x61` | SUBSYSTEM_BUSY_STATUS | ✅ OBSERVED 2026-07-08/09 (named per kdschlosser/ClimateTalk): sent **coordinator → thermostat**, value 0 ("not busy"), ~2 s after **operator-initiated** demand transitions (manual mode flips / setpoint changes at the stat). Never seen during automatic cycling — an equipment-readiness handshake, not a demand. |
