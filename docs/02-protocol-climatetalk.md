...
| `0x66` | FAN_DEMAND | `[12]=timer, [13]=mode, [14]=percent×2`. ✅ CONFIRMED: manual fan = mode 0, three discrete speeds **Low 25 % (0x32) / Med 50 % (0x64) / High 75 % (0x96)**; fan-on enters at Med. NOT honored during heat/cool — the IFC maps airflow to fire rate internally. **Directly observed 2026-07-12 (annotated capture):** manually toggling fan Hi/Off WHILE `COOL_DEMAND 30 %` was active — the stat DID put the `FAN_DEMAND` changes on the bus (0x96/0x00 interleaved with the live cool demand), but the physical blower stays at the cooling airflow; the manual speed only takes effect when IDLE. This is correct/by-design: dropping airflow below the coil's CFM/ton during compressor operation would freeze it. **TX consequence: SlyTherm must not command a fan speed to fight an active heat/cool call — circulate/manual-speed is an IDLE-only action.** |
